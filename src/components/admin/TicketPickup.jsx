...
import { getOrderItemsFromUUID, collectItems, getOrderFromLiUCardCode, getOrderFromEmail } from '../../api/ticketPickupCalls';
...
class TicketPickup extends Component {
  ...
  formSubmit = (values, bag) => {
    ...
    if (!values.blipp.includes('@')) {
      ...
      getOrderFromEmail(values.blipp)
        .then(res => {
          this.setState({ currUser: res.data, qrRead: false, showCollect: true });
          bag.setSubmitting(false);
        })
