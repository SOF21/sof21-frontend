...
export const DesktopCartPopup = withRouter(
  connect(mapStateToProps, { setShopPopupOpen, signOutUser })(
    UNCDesktopCartPopup
  )
)
...
export const MobileCartPopup = withRouter(
  connect(mapStateToProps, { setShopPopupOpen, signOutUser })(
    UNCMobileCartPopup
  )
)
...
class UNCCartPopupContent extends Component {
  ...
  render() {
    ...
    if (!isLoading && Object.keys(this.props.cart).length === 0) {
      ...
      Object.keys(this.props.cart).forEach((key) => {
        const baseProd =
          this.props.products[this.props.baseProducts[key].base_id]
        const productCost =
          baseProd.products[this.props.baseProducts[key].prod_id].actual_cost
    ...
    return (
      <React.Fragment>
        ...
        <Grid className='cart-middle-grid'>
          <GridInner>{!isLoading ? content : null}</GridInner>
        </Grid>
        {!isLoading && Object.keys(this.props.cart).length > 0 ? (
          <Grid
            ...
            style={
              !this.state.scrolledBottom
                ? { boxShadow: "0px 2px 2px 4px rgba(0, 0, 0, 0.14)" }
                : {}
            }
          >
            <GridCell desktop='12' tablet='8' phone='4'>
          ...
          <Grid style={{ paddingTop: "0px" }}></Grid>
        )}
      </React.Fragment>
    )
  }
...
export const CartPopupContent = injectIntl(
  withRouter(
